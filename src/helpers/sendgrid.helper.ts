...
const sendChangePassword = async (email: string, username: string, jwtToken: string ) => {
  await sendEmail(email, 'Solicitud de cambio de contraseña', `<p>Hola ${username},<br /> Se ha generado una solicitud de cambio de contraseña, para proceder a cambiarla haga click en el <a href="${process.env.FRONTEND_URL}/change-password?token=${jwtToken}">link</a> </p> <p>El enlace caducará pasados 30minutos.</p>`)
}
