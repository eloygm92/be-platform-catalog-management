...
export class WatchableService {
  ...
  async update(id: number, updateWatchableDto: UpdateWatchableDto) {
    ...
    const updated = await this.watchableRepository.update(id, updateWatchableDataDto);
    if(!providers)
  ...
  async remove(id: number) {
    ...
    const providersData = await this.getProvidersWithWatchable(providersAux.reduce((acc, provider) => {
      acc.push(provider.id);
      return acc;
    },[]));

    providersData.forEach(provider => provider.watchables = provider.watchables.filter(watchable => watchable.id !== id));
  ...
  private async saveProviders(watchable: Watchable, providers: number[]) {
    const providersWithWatchables = await this.getProvidersWithWatchable(providers);

    for await(const provider of providersWithWatchables) {
      if(!provider.watchables)
        provider.watchables = [watchable];
      else {
